...
table.addEventListener('click', (e)=>{
    ...
    if (currentColumnIndex === columnIndex) sortDirection *= -1;
    ...
    const sortedRows = rows.sort((rowA, rowB)=>{
        ...
        if (type === 'number') return (formatData(a) - formatData(b)) * sortDirection;
...
button.addEventListener('click', ()=>{
    ...
    for (const inp of form.elements){
        if (inp.name === 'name' && inp.value.length < 4) {
            pushNotification('Name must have more than 4 letters', 'Please provide a valid name with at least 4 characters', 'error');
            isValid = false;
        }
        if (inp.name === 'age' && (+inp.value < 18 || +inp.value > 90)) {
            pushNotification('Age must be from 18 to 90', 'Please enter a valid age between 18 and 90', 'error');
            isValid = false;
        }
    }
    if (!isValid) return;
    for (const inp of form.elements)if (inp.tagName === 'INPUT' || inp.tagName === 'SELECT') formData[inp.name] = inp.value;
    if (isValid) {
        ...
        for(const key in formData){
            ...
            } else newCell.textContent = formData[key];
        }
        pushNotification('Employee added successfully!', 'The employee has been added to the table.', 'success');
        form.reset();
    }
});
